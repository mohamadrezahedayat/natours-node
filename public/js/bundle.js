...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoibW9oYW1hZHJlemFoZWRheWF0IiwiYSI6ImNrcnNhcTZ4MDB1emoycHFteWJ6OTR4NjAifQ.1QfxJLnPlbID50wPqBfyPg';
  ...
  locations.forEach(function (loc) {
    ...
    el.className = 'marker'; // Add popup

    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); // add marker

    new mapboxgl.Marker({
      Element: el,
      anchor: 'bottom'
    }).setLngLat(loc.coordinates).addTo(map); // extend map nounds to include locations
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            _context.next = 3;
            return (0, _axios.default)("http://localhost:3200/api/v1/bookings/checkout-session/".concat(tourId));
...
var logoutBtm = document.querySelector('.nav__el--logout');
...
if (logoutBtm) logoutBtm.addEventListener('click', _login.logout);
